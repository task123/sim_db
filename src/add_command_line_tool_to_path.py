...
def add_path_to_bash_file(where_to_add_path):
    ...
    for line in bash_file.readlines():
        ...
        if line.split(' ')[0] == 'export':
            if (
                    len(line.split('/sim_db/command_line_tool')) == 2 and
                    line.split('/sim_db/command_line_tool')[1].strip() == ""):
...
def main():
    ...
    command_line_tool_dir = os.path.abspath(
            os.path.join(
                    os.path.dirname(
                            os.path.dirname(
                                    ...
                                    'command_line_tool'))))
    if os.sep == '/':
        command_line_tool = command_line_tool.replace(" ", "\ ")
    for program in ['sim_db', 'sdb', 'sim_db_cd_results.sh']:
        os.system("chmod u+x {0}".format(
                os.path.join(command_line_tool_dir, program)))
