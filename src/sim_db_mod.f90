...
type(sim_db) function sim_db_ctor(store_metadata) result(sim_database)
...
type(sim_db) function sim_db_ctor_with_id(id, store_metadata) &
        result(sim_database)
...
type(sim_db) function sim_db_ctor_without_search(path_proj_root, id, &
        store_metadata) result(sim_database)
...
function unique_results_dir_abs_path(self, path_to_dir)
    class(sim_db)::self
    character(len=*), intent(in) :: path_to_dir
