...
Connection::Connection(int id){
    sim_db = sim_db_ctor_with_id(id);
}

std::string Connection::make_subdir_result(std::string path_result_directory){
    std::string str(sim_db_make_subdir_result(sim_db, 
                                              path_result_directory.c_str()));
    return str;
}

void Connection::update_sha1_executables(std::vector<std::string> 
                                         paths_executables){
