...
class TemplateSpecializationHelper<std::vector<int> >{
    ...
    void write(struct SimDB* sim_db, std::string column, 
        ...
        sim_db_write_int_array(sim_db, column.c_str(), 
                               &value[0], value.size());
    }
    friend class Connection;
};
...
class TemplateSpecializationHelper<std::vector<double> >{
    ...
    void write(struct SimDB* sim_db, std::string column, 
        ...
        sim_db_write_double_array(sim_db, column.c_str(), 
                                  &value[0], value.size());
    }
    friend class Connection;
};
...
class TemplateSpecializationHelper<std::vector<std::string> >{
    ...
    void write(struct SimDB* sim_db, std::string column, 
        ...
        sim_db_write_string_array(sim_db, column.c_str(), 
                                  string_vec, value.size());
        for (int i = 0; i < value.size(); i++){
            delete[] string_vec[i];
        }
        delete[] string_vec;
...
class TemplateSpecializationHelper<std::vector<bool> >{
    ...
    void write(struct SimDB* sim_db, std::string column, 
        ...
        sim_db_write_bool_array(sim_db, column.c_str(), 
                                  bool_vec, value.size());
        delete[] bool_vec;
    }
    friend class Connection;
